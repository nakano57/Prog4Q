...
void write_pgm(const char *filename, unsigned char **cimg, int *x_size, int *y_size)
{
  FILE *ofp;
  int y;
  ...
  for (y = 0; y < 200; y++)
  {
    fwrite(cimg[y], sizeof(unsigned char), 200, ofp);
  }
  fclose(ofp);
  return;
}

void cutimage(unsigned char **img, unsigned char **cimg, int *x_size, int *y_size)
{
  int x, y;
...
int main(int argc, char const *argv[])
{
  ...
  {
    printf("ERROR!\n");
    printf("usage: clipimage_advenced [File Path] [Save Path]\n");
    printf("For example \"./clipimage_advenced ./fruit_460_460.pgm fruit_clipped.pgm\"\n");
    return 1;
  }

  fp = fopen(argv[1], "rb");

  cimg = (unsigned char **)malloc(sizeof(unsigned char *) * 200);
  for (i = 0; i < 200; i++)
  {
    cimg[i] = (unsigned char *)malloc(sizeof(unsigned char) * 200);
  }
  ...
  cutimage(img, cimg, x_size, y_size);

  write_pgm(argv[2], cimg, x_size, y_size);
